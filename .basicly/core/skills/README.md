...
| `no-comments` | `model` | any | Write and edit code files in a repository that bans prose comments — what to do instead of a comment, which comments are directives and stay, and the commands that report and remove them. Use whenever creating or editing any code file, and whenever the no-comments gate refuses a change. |
