...
This is the rule that gets skipped and the one that has cost the most. A design
document's *summary* of an interface is a claim about a claim. Read the implementation
...
Scope the search to the vendor so third-party summaries cannot outrank the source,
then fetch the specific page and ask a **narrow** question — exact flag names, exact
