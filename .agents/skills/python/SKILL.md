...
`module-size` refuses growth on a module already at its frozen token baseline, and since
the comment ban the count is code alone, so there is no prose left to trim in payment.
Paying it after the code is written is the slowest order. Before writing a line, measure
